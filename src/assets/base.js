...
google.devrel.samples.hello.CLIENT_ID =
    '934291292658-0bsot7ammlcec61eln4ejl7neqceje6v.apps.googleusercontent.com';
...
google.devrel.samples.hello.authedGreeting = function(id) {
	//gapi.client.helloworld.greetings.authed()
  gapi.client.duserendpoint.authenticate().execute(
      function(resp) {
        google.devrel.samples.hello.print(JSON.stringify(resp));
      });
};
...
google.devrel.samples.hello.init = function(apiRoot) {
  ...
  gapi.client.load('duserendpoint', 'v1', callback, apiRoot);
  gapi.client.load('dmatterendpoint', 'v1', callback, apiRoot);
  gapi.client.load('dtaskendpoint', 'v1', callback, apiRoot);
